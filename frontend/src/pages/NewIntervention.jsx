...
export default function NewIntervention() {
  ...
  const loadTipiIntervento = async () => {
    try {
      const response = await tipiInterventoAPI.getAll()
      setTipiIntervento(response.data.data || [])
    } catch (error) {
      toast.error('Errore caricamento tipi intervento')
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        tipoInterventoId: formData.tipoInterventoId,
  ...
  return (
    <div className="space-y-6">
      ...
      <h1 className="text-2xl font-bold text-gray-900">Nuovo Intervento</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <select
              required
              className="input"
              value={formData.tipoInterventoId}
              onChange={(e) => setFormData({ ...formData, tipoInterventoId: e.target.value })}
            >
