...
export default function Sopralluoghi() {
  ...
  const loadSopralluoghi = async () => {
    try {
      ...
      toast.error('Errore caricamento sopralluoghi')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h1 className="text-2xl font-bold text-gray-900">Sopralluoghi</h1>
        {user?.ruolo !== 'tecnico' && (
          <Link to="/sopralluoghi/new" className="btn-primary flex items-center justify-center space-x-2">
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {sopralluoghi.length === 0 ? (
          <div className="p-8 text-center text-gray-500">Nessun sopralluogo trovato</div>
        ) : (
          <div className="divide-y">
            {sopralluoghi.map((i) => (
              <Link
                key={i.id}
                to={`/sopralluoghi/${i.id}`}
